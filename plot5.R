...
vehiclesNEI <- NEI[NEI$SCC %in% SCC[grepl("vehicles", SCC$EI.Sector, ignore.case = T),]$SCC, ]

require(ggplot2,ggrepel)
## Subset by fips ==24510 and aggregate data by sum of emissions per year.
## Plot a line indicating if emissions have decreased or increase
p <- ggplot(aggregate(Emissions ~ year, subset(vehiclesNEI, fips == "24510"), sum)) + 
  aes(x = year, y = Emissions) + geom_point(color="#8b0000") + geom_line(color="#8b0000") 
...
p <- p + geom_text_repel(aes(label = sprintf("%0.1f", round(Emissions, digits = 1))),
                         size = 3, fontface = "bold", show.legend = FALSE, color="#8b0000")
